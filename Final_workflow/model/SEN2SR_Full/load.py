...
def trainable_model(path, device: str = "cuda", *args, **kwargs):
    ...
    sr_rgbn_model = MambaSR(**sr_parameters)  
    ...
    return rswir_modelx4(sr_rgbn_model, reference_srx2, sr_model, hard_constraint, device)



def compiled_model(path, device: str = "cuda", *args, **kwargs):
    ## RGBN 10m to 2.5m model ----------------------------------    
    sr_rgbn_model_weights = safetensors.torch.load_file(path / "sr_model.safetensor")
    sr_parameters = {
        "img_size": (128, 128),
...
def display_results(path: pathlib.Path, device: str = "cuda", *args, **kwargs):
